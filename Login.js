class Login {
        constructor() {
          this.generateSecretWord = createButton("Generate Play Code");
          this.message = createElement("h3");
          this.secretWord = createElement("h3");
        //   this.start = createButton('Start');
          this.number = createInput("Number of players");
        ...
        setElementPosition() {
          this.generateSecretWord.position(displayWidth/2 - 70, height / 2 - 100);
          this.message.position(width / 2.5, height / 2 - 230);
          this.secretWord.position(displayWidth/2 - 50, height / 2 - 200);
        }
        ...
        handleOnpress() {
          this.generateSecretWord.mousePressed(async () => {
            ...
            this.message.html("Share code with friends 2-6 players");
            this.secretWord.html(`Code: ${secret_word}`);
            this.number.position(width/2.3, height/2 - 80);
      
            this.nameInput.position(width / 2.3, height / 2 - 120);
            this.playButton.position(width/2.2, height / 2 - 60);
          });
          ...
          this.playButton.mousePressed(() => {
            ...
            this.greeting.position(width / 2 - 50, height / 4);
      
            this.greeting2.html("Waiting for other players to join ....");
            this.greeting2.position(width / 2.5, height / 3.2);
            // this.start.position(width/2, height/1.7);
            this.secretWord.position(width / 2.2, height / 2.5);
          });
        }
